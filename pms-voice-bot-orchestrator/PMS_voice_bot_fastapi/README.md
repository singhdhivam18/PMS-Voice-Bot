...
The current agent uses these exact dynamic-variable names (names are case-sensitive):
...
Carexpotel now sends those names exactly when it starts the outbound call. ElevenLabs documents dynamic-variable names as case-sensitive.
...
Add these values (the agent and phone-number IDs from your current configuration are already placed in `.env.example`):
...
## 9. Important agent configuration note

Your current `conversation_template` references `{{Input_date}}` and `{{Input_Time}}`, while the configured Data Collection items are named `Appointment_Date` and `Appointment_Time`. Data Collection extracts values after the conversation; it does not automatically make those values available as runtime prompt variables during the call.

For the first integration test, focus on the outbound call and transcript. After that test works, update the agent conversation logic so the confirmation sentence uses the values that are actually available at that point in the conversation, or add a tool/state step that writes the collected values into runtime variables.

Your current agent also already has `post_call_transcription` enabled at the workspace/agent configuration level with JSON transcript delivery. Carexpotel's webhook should therefore point to `/api/webhooks/elevenlabs/post-call`.
